...
public class FOM_DonatorList
{
    ...
    private static List<String> superDonatorNames = new ArrayList();
    ...
    public static boolean isFullDonator(CommandSender user)
    {
        return isFullDonator(user, false);
    }

    public static boolean isFullDonator(CommandSender user, boolean verify_is_donator)
    {
        if (verify_is_donator)
        {
            if (!isUserDonator(user))
        ...
        if (!(user instanceof Player))
        {
            return fullDonatorNames.contains(user_name);
        }
        FOM_Donator donator_entry = getDonatorEntry((Player) user);
        if (donator_entry != null)
        {
            return donator_entry.isFullDonator();
        }
        return false;
    }
    ...
    public static void addDonator(String donator_name, List<String> ips)
    {
        try
        {
            ...
            else
            {
                ...
                FOM_Donator donator = new FOM_Donator(donator_name, ips, last_login, custom_login_message, is_super_donator, is_senior_donator, is_full_donator, console_aliases, true);
                donatorList.put(donator_name.toLowerCase(), donator);
            }
            saveDonatorList();
        }
